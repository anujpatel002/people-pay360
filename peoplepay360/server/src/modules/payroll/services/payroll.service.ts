...
async function ensureVersion(structureId: string | undefined | null, actor: Actor) {
  let targetStructureId = structureId ? String(structureId).trim() : '';
  
  if (!targetStructureId) {
    const [structures] = await pool.execute<any[]>('SELECT id FROM salary_structures WHERE is_active=1 ORDER BY id ASC LIMIT 1');
    if (!structures[0]) {
      // Create default salary structure if none exists
      const defaultId = 'd0000000-0000-0000-0000-000000000001';
      await pool.execute('INSERT IGNORE INTO salary_structures (id, name, is_active) VALUES (?, ?, 1)', [defaultId, 'Regular Salary Structure']);
      targetStructureId = defaultId;
    } else {
      targetStructureId = structures[0].id;
    }
  }

  // 1. Check if active version already exists
  const [versions] = await pool.execute<any[]>('SELECT * FROM salary_structure_versions WHERE structure_id=? AND status=? ORDER BY version DESC LIMIT 1', [targetStructureId, 'Active']);
  if (versions[0]) return versions[0];

  // 2. Validate structure exists
  const [structures] = await pool.execute<any[]>('SELECT * FROM salary_structures WHERE (id=? OR name=?) AND is_active=1 LIMIT 1', [targetStructureId, targetStructureId]);
  if (!structures[0]) {
    const [fallback] = await pool.execute<any[]>('SELECT * FROM salary_structures WHERE is_active=1 ORDER BY id ASC LIMIT 1');
    if (!fallback[0]) throw new ValidationError('Invalid or inactive Salary Structure');
    targetStructureId = fallback[0].id;
  } else {
    targetStructureId = structures[0].id;
  }

  // Check version again with resolved targetStructureId
  const [existingVer] = await pool.execute<any[]>('SELECT * FROM salary_structure_versions WHERE structure_id=? AND status=? ORDER BY version DESC LIMIT 1', [targetStructureId, 'Active']);
  if (existingVer[0]) return existingVer[0];

  // 3. Create version 1 and copy rules
  const id = crypto.randomUUID();
  await pool.execute('INSERT INTO salary_structure_versions (id,structure_id,version,status,created_by) VALUES (?,?,1,?,?)', [id, targetStructureId, 'Active', actor.id]);
  const [rules] = await pool.execute<any[]>('SELECT * FROM salary_rules WHERE structure_id=? AND is_active=1 ORDER BY sequence,id', [targetStructureId]);
  for (const rule of rules) {
    await pool.execute(`INSERT INTO salary_rule_versions
      ...
      VALUES (UUID(),?,?,?,?,?,?,?,?,?,?,?,?)`, [
        ...
        rule.amount,
        rule.percentage,
        rule.percentage_base_type ?? (rule.computation_method === 'percentage_of_gross' ? 'GROSS' : null),
        rule.percentage_base_code ?? null,
        rule.formula,
        rule.is_active,
      ] as any[]);
  }
  return { id, structure_id: targetStructureId, version: 1 };
}
...
export async function createPayrun(data: any, actor: Actor) {
  if (!Array.isArray(data.employeeIds) || !data.employeeIds.length) {
  ...
  const resolvedStructureId = version.structure_id;

  // Resolve employees from provided IDs/employee numbers
  const inputKeys = [...new Set<string>((data.employeeIds as (string | number)[]).map((s) => String(s).trim()).filter(Boolean))];
  const resolvedEmployees: any[] = [];

  for (const rawKey of inputKeys) {
    const cleanKey = rawKey.replace(/^#/, '').trim();
    const numOnly = cleanKey.replace(/^EMP-?/i, '');

    const [employees] = await pool.execute<any[]>(
      `SELECT * FROM employees 
       WHERE (id = ? OR employee_number = ? OR employee_number = ? OR employee_number = ? OR employee_number = ?)
       LIMIT 1`,
      [cleanKey, cleanKey, `EMP-${cleanKey}`, `EMP-${numOnly}`, numOnly]
    );

    if (employees[0]) {
      if (!resolvedEmployees.some((e) => e.id === employees[0].id)) {
        resolvedEmployees.push(employees[0]);
      }
    }
  }

  if (resolvedEmployees.length === 0) {
    throw new ValidationError('None of the specified employee IDs / numbers were found in active roster');
  }

  const id = crypto.randomUUID();
  const companyId = data.companyId || resolvedEmployees[0]?.company_id || null;

  await pool.execute(
    `INSERT INTO payruns (id,name,period_id,company_id,period_start,period_end,structure_id,structure_version_id,status,employee_count,currency_code)
     VALUES (?,?,?,?,?,?,?,?, 'Draft',?,?)`,
    [
      id,
      data.name,
  ...
  for (const e of resolvedEmployees) {
    let contract: any = null;
    try {
      contract = await resolveActiveContract(e.id, data.periodStart, data.periodEnd);
    } catch {
      /* warning below preserves the draft */
    }

    await pool.execute(
      `INSERT INTO payslips (id,payrun_id,employee_id,contract_id,structure_id,structure_version_id,employee_name_snapshot,employee_code_snapshot,department_snapshot,job_position_snapshot,contract_reference_snapshot,currency_code,warning_codes)
      ...
      [
        ...
        e.employee_number,
        contract?.department ?? e.department_id ?? null,
        contract?.jobPosition ?? e.job_title ?? null,
...
export async function getPayrun(id: string) {
  ...
  const [payslips] = await pool.execute<any[]>(
    `SELECT p.*,
            COALESCE(NULLIF(p.employee_name_snapshot, ''), CONCAT(e.first_name, ' ', e.last_name), 'Employee') AS employee_name_snapshot,
            COALESCE(NULLIF(p.employee_code_snapshot, ''), e.employee_number, 'EMP') AS employee_code_snapshot,
            e.avatar_url,
            e.first_name,
            e.last_name,
            e.work_email,
  ...
  const [warnings] = await pool.execute<any[]>(
    `SELECT w.*,
            COALESCE(NULLIF(p.employee_name_snapshot, ''), CONCAT(e.first_name, ' ', e.last_name), 'Employee') AS employee_name,
            COALESCE(NULLIF(p.employee_code_snapshot, ''), e.employee_number, 'EMP') AS employee_number,
            e.avatar_url,
            d.name AS department_name
     FROM payroll_warnings w
     LEFT JOIN payslips p ON p.id = w.payslip_id
     LEFT JOIN employees e ON e.id = w.employee_id
     LEFT JOIN departments d ON d.id = e.department_id
     WHERE w.payrun_id = ?
     ORDER BY w.blocking DESC, w.created_at ASC`,
    [id]
  );
...
async function replaceWarnings(runId: string, slips: any[]) {
  ...
  const [countRows] = await pool.execute<any[]>('SELECT COUNT(*) count FROM payroll_warnings WHERE payrun_id=? AND status=?', [runId, 'OPEN']);
  const warnCount = countRows[0]?.count ?? 0;
  await pool.execute('UPDATE payruns SET warning_count=? WHERE id=?', [warnCount, runId]);
}
...
export async function computePayrun(id: string, actor: Actor, recompute = false) {
  ...
  for (const slip of slips) {
    ...
    const [attendanceRows] = await pool.execute<any[]>(`SELECT COALESCE(SUM(worked_minutes),0) worked, COALESCE(SUM(overtime_minutes),0) overtime, COUNT(DISTINCT date) days FROM attendance_records WHERE employee_id=? AND date BETWEEN ? AND ? AND check_out IS NOT NULL`, [slip.employee_id, run.period_start, run.period_end]);
    const attendance = attendanceRows[0] || { worked: 0, overtime: 0, days: 0 };
    const [leaveRows] = await pool.execute<any[]>(`SELECT COALESCE(SUM(CASE WHEN t.is_paid=0 THEN r.days ELSE 0 END),0) unpaid FROM time_off_requests r JOIN time_off_types t ON t.id=r.type_id WHERE r.employee_id=? AND r.status IN ('Confirmed','Approved') AND r.start_date<=? AND r.end_date>=?`, [slip.employee_id, run.period_end, run.period_start]);
    const leave = leaveRows[0] || { unpaid: 0 };
    ...
    const vars: Record<string, number> = {
      'contract.wage': num(contract?.wage),
      wage: num(contract?.wage),
      WAGE: num(contract?.wage),
      worked_hours: num(attendance.worked) / 60,
      overtime_hours: num(attendance.overtime) / 60,
      worked_days: num(attendance.days),
      calendar_days: Math.floor((Date.parse(run.period_end) - Date.parse(run.period_start)) / 86400000) + 1,
      unpaid_leave: num(leave.unpaid),
    };
    Object.entries(inputMap).forEach(([k, v]) => {
      vars[`inputs.${k}`] = v;
      vars[k] = v;
    });
    await pool.execute('DELETE FROM payslip_calculation_traces WHERE payslip_id=?', [slip.id]);
    await pool.execute('DELETE FROM payslip_lines WHERE payslip_id=?', [slip.id]);
    let gross = 0, deductions = 0, employer = 0;
    for (const rule of rules) {
      let base = 0;
      if (rule.computation_method === 'fixed_amount') {
        base = num(rule.amount);
      } else if (rule.computation_method === 'percentage_of_gross') {
        const baseType = rule.percentage_base_type ?? 'GROSS';
        base =
          baseType === 'BASIC'
            ? (vars['rules.BASIC'] ?? vars['BASIC'] ?? 0)
            : baseType === 'RULE'
            ? (vars[`rules.${rule.percentage_base_code}`] ?? vars[`${rule.percentage_base_code}`] ?? 0)
            : baseType === 'INPUT'
            ? (vars[`inputs.${rule.percentage_base_code}`] ?? vars[`${rule.percentage_base_code}`] ?? 0)
            : gross;
        base = (base * num(rule.percentage)) / 100;
      } else {
        base = evaluateFormula(rule.formula ?? '0', {
          ...vars,
          GROSS: gross,
          'rules.GROSS': gross,
          NET: gross - deductions,
          'rules.NET': gross - deductions,
        });
      }
      const amount = round(base);
      vars[`rules.${rule.rule_code}`] = amount;
      vars[rule.rule_code] = amount;
      if (rule.category === 'Deduction') deductions += amount;
      else if (rule.category === 'EmployerContribution') employer += amount;
      else if (!['Net', 'Gross'].includes(rule.category)) gross += amount;
...
export async function validatePayrun(id: string, actor: Actor) {
  ...
  const [slipsRows] = await pool.execute<any[]>('SELECT COUNT(*) count FROM payslips WHERE payrun_id=?', [id]);
  if (!slipsRows[0]?.count) throw new ValidationError('Payrun contains no payslips');
  const [blockingRows] = await pool.execute<any[]>('SELECT COUNT(*) count FROM payroll_warnings WHERE payrun_id=? AND blocking=1 AND status=?', [id, 'OPEN']);
  if (blockingRows[0]?.count) throw new ValidationError('Resolve blocking payroll warnings before validation');
  const [mismatchRows] = await pool.execute<any[]>(`SELECT COUNT(*) count FROM payslips p LEFT JOIN (SELECT payslip_id,SUM(CASE WHEN category NOT IN ('Deduction','EmployerContribution','Net','Gross') THEN amount ELSE 0 END) earnings,SUM(CASE WHEN category='Deduction' THEN amount ELSE 0 END) deductions FROM payslip_lines GROUP BY payslip_id) l ON l.payslip_id=p.id WHERE p.payrun_id=? AND (ROUND(p.gross,2)<>ROUND(COALESCE(l.earnings,0),2) OR ROUND(p.deductions,2)<>ROUND(COALESCE(l.deductions,0),2))`, [id]);
  if (mismatchRows[0]?.count) throw new ValidationError('Computation mismatch detected');
...
export async function resolveWarning(id: string, note: string, actor: Actor) {
  await pool.execute(`UPDATE payroll_warnings SET status='RESOLVED',resolution_note=?,resolved_by=?,resolved_at=NOW() WHERE id=? AND status='OPEN'`, [note ?? 'Acknowledged & resolved by payroll manager', actor.id, id]);
  const [rows] = await pool.execute<any[]>('SELECT payrun_id FROM payroll_warnings WHERE id=?', [id]);
  if (!rows[0]) throw new NotFoundError('Warning not found');
  const [countRows] = await pool.execute<any[]>('SELECT COUNT(*) count FROM payroll_warnings WHERE payrun_id=? AND status=?', [rows[0].payrun_id, 'OPEN']);
  await pool.execute('UPDATE payruns SET warning_count=? WHERE id=?', [countRows[0]?.count ?? 0, rows[0].payrun_id]);
  await audit('Payrun', rows[0].payrun_id, 'WARNING_RESOLVED', actor, undefined, undefined, { warningId: id, resolutionNote: note });
}

export async function getPayslip(id: string) {
  const [s] = await pool.execute<any[]>(
    `SELECT p.*,
            COALESCE(NULLIF(p.employee_name_snapshot, ''), CONCAT(e.first_name, ' ', e.last_name), 'Employee') AS employee_name_snapshot,
            COALESCE(NULLIF(p.employee_code_snapshot, ''), e.employee_number, 'EMP') AS employee_code_snapshot,
            e.avatar_url,
  ...
  if (!s[0]) throw new NotFoundError('Payslip not found');
  const [lines] = await pool.execute<any[]>('SELECT * FROM payslip_lines WHERE payslip_id=? ORDER BY sequence', [id]);
  const [trace] = await pool.execute<any[]>('SELECT * FROM payslip_calculation_traces WHERE payslip_id=? ORDER BY sequence', [id]);
  const [warnings] = await pool.execute<any[]>('SELECT * FROM payroll_warnings WHERE payslip_id=?', [id]);
  return { ...s[0], lines, trace, warnings };
}

export async function listPayslips(query: any) {
  const where = query.payrunId ? 'WHERE p.payrun_id=?' : '';
  const [rows] = await pool.execute<any[]>(
    `SELECT p.*,
     ...
     ${where}
     ORDER BY employee_name_snapshot ASC`,
    query.payrunId ? [query.payrunId] : []
  );
  return { data: rows };
}

export async function deletePayrun(id: string, actor: Actor) {
  const run = await loadRun(id);
  const [payslips] = await pool.execute<any[]>('SELECT id FROM payslips WHERE payrun_id = ?', [id]);
  const slipIds = payslips.map((p) => p.id);

  if (slipIds.length > 0) {
    const placeholders = slipIds.map(() => '?').join(',');
    await pool.execute(`DELETE FROM payslip_calculation_traces WHERE payslip_id IN (${placeholders})`, slipIds);
    await pool.execute(`DELETE FROM payslip_lines WHERE payslip_id IN (${placeholders})`, slipIds);
  }

  await pool.execute('DELETE FROM payroll_warnings WHERE payrun_id = ?', [id]);
  await pool.execute('DELETE FROM payroll_inputs WHERE payrun_id = ?', [id]);
  await pool.execute('DELETE FROM payroll_payments WHERE payrun_id = ?', [id]);
  await pool.execute('DELETE FROM payslip_deliveries WHERE payrun_id = ?', [id]);
  await pool.execute('DELETE FROM payslips WHERE payrun_id = ?', [id]);
  await pool.execute('DELETE FROM payroll_audit_logs WHERE entity_id = ?', [id]);
  await pool.execute('DELETE FROM payruns WHERE id = ?', [id]);

  await audit('Payrun', id, 'DELETED', actor, run.status, undefined, {
    name: run.name,
...
export async function sendPayslips(runId: string, actor: Actor, retry = false) {
  const run = await loadRun(runId);
  if (run.status !== 'Paid') throw new AppError(409, 'Payslips can only be delivered after payment');
  const [slips] = await pool.execute<any[]>(`SELECT p.*,e.work_email FROM payslips p JOIN employees e ON e.id=p.employee_id WHERE p.payrun_id=?`, [runId]);
  const results = [];
  for (const s of slips) {
    const [existing] = await pool.execute<any[]>('SELECT * FROM payslip_deliveries WHERE payslip_id=?', [s.id]);
    ...
    const failed = !s.work_email;
    await pool.execute(
      `INSERT INTO payslip_deliveries(id,payrun_id,payslip_id,employee_id,recipient_email,status,attempt_count,error_message,last_attempt_at,sent_at)
       VALUES(UUID(),?,?,?,?,?,1,?,NOW(),?)
       ON DUPLICATE KEY UPDATE status=VALUES(status),attempt_count=attempt_count+1,error_message=VALUES(error_message),last_attempt_at=NOW(),sent_at=VALUES(sent_at)`,
      [runId, s.id, s.employee_id, s.work_email ?? '', failed ? 'Failed' : 'Sent', failed ? 'Missing recipient email' : null, failed ? null : new Date()] as any[]
    );
    results.push({ payslipId: s.id, status: failed ? 'Failed' : 'Sent' });
  }
  await audit('Payrun', runId, retry ? 'DELIVERY_RETRIED' : 'PAYSLIPS_SENT', actor, undefined, undefined, { results });
  return results;
}

export async function createPeriod(data: any, actor: Actor) {
  if (data.periodEnd < data.periodStart) throw new ValidationError('Invalid payroll period');
  const id = crypto.randomUUID();
  await pool.execute(`INSERT INTO payroll_periods(id,name,company_id,period_start,period_end,payment_date,frequency,created_by) VALUES(?,?,?,?,?,?,?,?)`, [
    id,
    data.name,
    data.companyId ?? null,
    data.periodStart,
...
export async function listPeriods() {
  const [rows] = await pool.execute<any[]>('SELECT * FROM payroll_periods ORDER BY period_end DESC');
  return { data: rows };
}
