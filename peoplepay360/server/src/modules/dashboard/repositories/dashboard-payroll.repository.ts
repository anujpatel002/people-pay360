...
export async function getPayrollTotals(filters: DashboardFilters): Promise<{
  ...
  const [[row]] = await pool.execute<(RowDataPacket & { total_net: number; payslip_count: number })[]>(`
    SELECT
...
export async function countMissingBankDetails(companyId?: string): Promise<{ employeeId: string }[]> {
  ...
  const [rows] = await pool.execute<(RowDataPacket & { id: string })[]>(
    `SELECT id FROM employees WHERE ${conditions.join(' AND ')}`,
    params as any[]
  );
  return rows.map(r => ({ employeeId: r.id }));
}

/** Detect duplicate payslips: employee has a payslip in two overlapping payruns. */
export async function findDuplicatePayslips(companyId?: string): Promise<{ employeeId: string; payrunId: string }[]> {
  const companyFilter = companyId ? 'AND r.company_id = ?' : '';
  const params = companyId ? [companyId] : [];
  const [rows] = await pool.execute<(RowDataPacket & { employee_id: string; payrun_id: string })[]>(`
    SELECT p.employee_id, p.payrun_id
    FROM payslips p
    JOIN payruns r ON r.id = p.payrun_id
    ...
    GROUP BY p.employee_id, p.payrun_id
  `, params as any[]);
  return rows.map(r => ({ employeeId: r.employee_id, payrunId: r.payrun_id }));
}

/** Find unvalidated payruns (Draft or Computed). */
export async function findUnvalidatedPayruns(companyId?: string): Promise<{ payrunId: string; status: string }[]> {
  const conditions = ["status IN ('Draft','Computed')"];
  const params: unknown[] = [];
  if (companyId) { conditions.push('company_id = ?'); params.push(companyId); }
  const [rows] = await pool.execute<(RowDataPacket & { id: string; status: string })[]>(
    `SELECT id, status FROM payruns WHERE ${conditions.join(' AND ')}`,
    params as any[]
  );
  return rows.map(r => ({ payrunId: r.id, status: r.status }));
}
